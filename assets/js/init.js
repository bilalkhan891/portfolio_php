...
function sting_tm_contact_form(){
	...
	jQuery(".contact_form #send_message").on('click', function(){
		...
		if(name===''||email===''||message===''){
			...
			jQuery.post("index-4.php",{ ajax_name: name, ajax_email: email, ajax_message:message, ajax_subject: subject}, function(data) {
...
function sting_tm_animate_text(){
	...
	var baName 				=  jQuery('.ba_name').text();
	var baExperties 		=  jQuery('.ba_experties').text();
	
		animateSpan.typed({
			strings: [ baName, baExperties],
			loop: true,
			startDelay: 1e3,
			backDelay: 2e3
		});
